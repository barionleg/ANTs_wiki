We are frequently asked what the default `ANTS` parameters would look like in a call to `antsRegistration`. There is not a one to one mapping between the two programs, but some options can be set in a similar way. However, even when the two programs use theoretically similar algorithms, equivalence cannot be achieved because the underliying implementations on both the ANTs and ITK side have evolved over the years. For example, Gaussian smoothing is not computed the same way in `antsRegistration` as it was in `ANTS`.
